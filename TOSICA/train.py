...
def splitDataSet(adata,label_name='Celltype', tr_ratio= 0.7): 
    ...
    exp_train = torch.from_numpy(np.array(train_dataset)[:,:n_genes].astype(np.float32)).cpu().numpy()
    label_train = torch.from_numpy(np.array(train_dataset)[:,-1].astype(np.int64)).cpu().numpy()
    exp_valid = torch.from_numpy(np.array(valid_dataset)[:,:n_genes].astype(np.float32)).cpu().numpy()
    label_valid = torch.from_numpy(np.array(valid_dataset)[:,-1].astype(np.int64)).cpu().numpy()
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        _,outputs,_ = model(exp.to(device))
...
def evaluate(model, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        labels_cpu = labels.cpu().numpy()  # Ensure labels are also on CPU

        accu_num += torch.eq(pred_classes, labels.to(device)).sum()

        # Calculate F1 score
        f1 = f1_score(pred_classes_cpu, labels_cpu, average=None)
        f1_avg += np.median(f1)
    ...
    return accu_loss.item() / (step + 1), accu_num.item() / sample_num, f1_avg.item() / sample_num




        
    #to do : check f1 calculated is correct?
    f1 = f1_score(pred_classes, labels.to(device), average=None)
    f1_avg += np.median(f1).cpu().numpy()
...
def fit_model(adata, gmt_path, project = None, pre_weights='', label_name='Celltype',max_g=300,max_gs=300, mask_ratio = 0.015,n_unannotated = 1,batch_size=8, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01):
    ...
    label_train = torch.tensor(label_train, dtype=torch.int64)
    #if isinstance(label_train, np.ndarray): 
    #else label_train
    num_classes = torch.max(label_train) + 1
    ...
    for epoch in range(epochs):
        train_loss, train_acc = train_one_epoch(model=model,
                                                ...
                                                epoch=epoch)
        scheduler.step() 
        val_loss, val_acc, val_f1_avg = evaluate(model=model,
